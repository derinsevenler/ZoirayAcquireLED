function [out] = CAMERA_Prosilica(options)
% [out] = CAMERA_Prosilica(options) has one input argument and one output
...
if (quickexit == 0) %continue
    ...
    % view the DeviceInfo structure for the device that is returned by
    % the imaqhwinfo function. Each DeviceInfo structure contains a
    % SupportedFormats field.
    prosilica_formats = {};
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    if(capture_mode==0)
        vid_obj = videoinput('qimaging',1,prosilica_formats{5});
    else
        vid_obj = videoinput('qimaging',1,prosilica_formats{1});
    end
